...
async function boot(history = {}, options = {}) {
	...
	</main></body></html>`, { url: options.url || 'https://example.com/', runScripts: 'outside-only' });
	const { window } = dom;
...
test('history total includes stored posts absent from the loaded feed', async () => {
 const now = Math.floor(Date.now() / 1000);
...
test('Show mode survives the archive restart without hiding or skipping old posts', async () => {
 const now=Math.floor(Date.now()/1000);let requests=0;
 const {window,loadMoreClicks}=await boot({1:now,2:now,3:now,4:now},{
  url:'https://example.com/#wp-seen-all',hasMorePages:true,unseenPrefetchPageLimit:6,
  homePostIndex:[1,2,3,4,5,6],maxPages:3,
  beforeEval(w){
   w.fetch=async()=>{requests++;};
   w.eval(earlyHide);
   assert.equal(w.WPSeenPostsEarlyHide,undefined);
  }
 });
 assert.equal(window.document.querySelector('.wp-seen-posts-toggle').textContent,'Hide seen');
 assert.equal(window.document.querySelectorAll('.wp-seen-posts-is-hidden').length,0);
 assert.equal(window.document.querySelectorAll('.wp-seen-posts-reload-preview').length,0);
 assert.equal(requests,0);assert.equal(loadMoreClicks(),0);
 assert.equal(window.location.hash,'');
 const card=window.document.createElement('li');card.id='prologue-3';card.className='post';
 const feed=window.document.querySelector('#postlist');feed.appendChild(card);
 window.document.dispatchEvent(new window.CustomEvent('wpFeedPostsAdded',{detail:{container:feed,posts:[card]}}));
